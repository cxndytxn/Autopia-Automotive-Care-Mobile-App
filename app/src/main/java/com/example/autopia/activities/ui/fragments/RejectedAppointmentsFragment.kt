...
import com.example.autopia.activities.adapter.RejectedAppointmentsAdapter
import com.example.autopia.activities.adapter.WorkshopRejectedAppointmentsAdapter
...
class RejectedAppointmentsFragment : Fragment() {
    ...
    private lateinit var userAppointmentListAdapter: RejectedAppointmentsAdapter
    private var workshopAppointmentList: MutableList<Appointments> = mutableListOf()
    private lateinit var workshopAppointmentListAdapter: WorkshopRejectedAppointmentsAdapter
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_rejected_appointments, container, false)
    }
    ...
    private fun loadData() {
        val recyclerView: RecyclerView? =
            parentFragment?.requireActivity()?.findViewById(R.id.rejected_appointment_rv)
                    ...
                    if (view != null) {
                        if (userType == "user") {
                            ...
                            if (isAdded) {
                                ...
                                viewModel.getRejectedAppointmentsByClientId(user.uid)
                                viewModel.appointments.observe(viewLifecycleOwner) { response ->
                                    //pass list here
                                    if (response.isSuccessful && !response.body().isNullOrEmpty()) {
                                        appointmentList = response.body()!!
                                        userAppointmentListAdapter = RejectedAppointmentsAdapter(
                                        ...
                                        val noAppointmentLayout: ConstraintLayout? =
                                            requireActivity().findViewById(R.id.noRejectedAppointmentLayout)
                            ...
                            if (isAdded) {
                                ...
                                viewModel.getRejectedAppointmentsByWorkshopId(user.uid)
                                viewModel.appointments.observe(viewLifecycleOwner) { response ->
                                    //pass list here
                                    if (response.isSuccessful && !response.body().isNullOrEmpty()) {
                                        appointmentList = response.body()!!
                                        workshopAppointmentListAdapter =
                                            WorkshopRejectedAppointmentsAdapter(
                                                this.requireActivity(),
