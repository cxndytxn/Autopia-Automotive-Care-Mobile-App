...
class PendingAppointmentsFragment : Fragment() {
    ...
    private lateinit var userAppointmentListAdapter: PendingAppointmentsAdapter
    private var workshopAppointmentList: MutableList<Appointments> = mutableListOf()
    private lateinit var workshopAppointmentListAdapter: WorkshopPendingAppointmentsAdapter
    ...
    private fun loadData() {
        val recyclerView: RecyclerView? =
            parentFragment?.requireActivity()?.findViewById(R.id.pending_appointment_rv)
                ...
                if (it.isSuccessful) {
                    ...
                    if (userType == "user") {
                        ...
                        if (isAdded) {
                                ...
                                viewModel.getPendingAppointmentsByClientId(user.uid)
                            if (view != null) {
                                viewModel.appointments.observe(viewLifecycleOwner) { response ->
                                    //pass list here
                                    if (response.isSuccessful && !response.body().isNullOrEmpty()) {
                                        appointmentList = response.body()!!
                                        userAppointmentListAdapter = PendingAppointmentsAdapter(
                                        ...
                                        val noAppointmentLayout =
                                            requireActivity().findViewById<ConstraintLayout>(R.id.noPendingAppointmentLayout)
                        ...
                        if (isAdded) {
                            if (view != null) {
                                ...
                                viewModel.getPendingAppointmentsByWorkshopId(user.uid)
                                viewModel.appointments.observe(viewLifecycleOwner) { response ->
                                    //pass list here
                                    if (response.isSuccessful && !response.body().isNullOrEmpty()) {
                                        appointmentList = response.body()!!
                                        workshopAppointmentListAdapter =
                                            WorkshopPendingAppointmentsAdapter(
                                                this.requireActivity(),
