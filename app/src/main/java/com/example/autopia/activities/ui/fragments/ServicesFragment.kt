...
import com.example.autopia.activities.adapter.ServicesAdapter
...
class ServicesFragment : Fragment() {
    ...
    private lateinit var serviceListAdapter: ServicesAdapter
    ...
    override fun onCreateView(
        ...
        if (user != null) {
            ...
            root.findViewById<FloatingActionButton>(R.id.add_service_button).setOnClickListener {
                val navController =
                    requireActivity().findNavController(R.id.workshop_nav_host_fragment)
                navController.navigate(R.id.addServiceFragment)
            }
        } else {
            "Hello there, you can't manage services when you're not logged in!".also {
                not_logged_in_vehicles.text = it
            }
        }

        serviceListAdapter = ServicesAdapter(this.requireActivity(), serviceList)
        ...
        if (view != null) {
            ...
            viewModel.services.observe(viewLifecycleOwner) { response ->
                ...
                if (response.isSuccessful) {
                    ...
                    serviceListAdapter = ServicesAdapter(this.requireActivity(), serviceList)
                    val recyclerView: RecyclerView? = requireView().findViewById(R.id.service_rv)
