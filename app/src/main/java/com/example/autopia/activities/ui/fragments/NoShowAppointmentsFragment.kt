...
import com.example.autopia.activities.adapter.NoShowAppointmentsAdapter
import com.example.autopia.activities.adapter.PendingAppointmentsAdapter
import com.example.autopia.activities.adapter.WorkshopNoShowAppointmentsAdapter
import com.example.autopia.activities.adapter.WorkshopPendingAppointmentsAdapter
...
class NoShowAppointmentsFragment : Fragment() {
    ...
    private lateinit var userAppointmentListAdapter: NoShowAppointmentsAdapter
    private var workshopAppointmentList: MutableList<Appointments> = mutableListOf()
    private lateinit var workshopAppointmentListAdapter: WorkshopNoShowAppointmentsAdapter
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_no_show_appointments, container, false)
    }
    ...
    private fun loadData() {
        val recyclerView: RecyclerView? =
            parentFragment?.requireActivity()?.findViewById(R.id.no_show_appointment_rv)
                    ...
                    if (userType == "user") {
                        if (view != null) {
                            ...
                            if (isAdded) {
                                ...
                                viewModel.getNoShowAppointmentsByClientId(user.uid)
                                viewModel.appointments.observe(viewLifecycleOwner) { response ->
                                    //pass list here
                                    if (response.isSuccessful && !response.body().isNullOrEmpty()) {
                                        appointmentList = response.body()!!
                                        userAppointmentListAdapter = NoShowAppointmentsAdapter(
                                            requireActivity(),
                                            userAppointmentList
                                        )
                                        ...
                                        val noAppointmentLayout =
                                            requireActivity().findViewById<ConstraintLayout>(R.id.noShowAppointmentLayout)
                        ...
                        if (view != null) {
                            ...
                            if (isAdded) {
                                ...
                                viewModel.getNoShowAppointmentsByWorkshopId(user.uid)
                                viewModel.appointments.observe(viewLifecycleOwner) { response ->
                                    //pass list here
                                    if (response.isSuccessful && !response.body().isNullOrEmpty()) {
                                        appointmentList = response.body()!!
                                        workshopAppointmentListAdapter =
                                            WorkshopNoShowAppointmentsAdapter(
